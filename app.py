...
st.markdown("""
    ...
    /* Ограничиваем высоту стартовой картинки, чтобы она идеально влезала в экран без прокрутки */
    ...
    if user_vin_input:
        ...
        if not user_cars.empty:
            ...
            if view_to_lead_ratio < 5.0:
                st.progress(0.35)
                st.markdown("🔴 **Статус:** `Нужно скорректировать цену` — Просмотры есть, но откликов (звонков/сообщений) критически мало. Рынок считает цену завышенной. Рекомендуется обсудить снижение стоимости с вашим менеджером для запуска active продаж.")
            ...
            st.subheader("🧏‍♂️ Сопровождение сделки")
            ...
            with buyout_col:
                ...
                st.markdown(f"""
                    <div class="buyout-container">
                        ...
                        <p>Устали ждать покупателя или срочно нужны деньги? Автосалон Гусар готов выкупить Ваш автомобиль прямо сегодня за сумму:</p>
