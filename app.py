...
try:
    ...
    user_dogovor = st.text_input("Введите номер вашего договора комиссии (например, К-102):", "").strip()
    
    if user_dogovor:
        # Ищем машины по введенному номеру договора
        user_cars = df_cars[df_cars['Номер договора комиссии'].astype(str).str.lower() == user_dogovor.lower()]
        
        if not user_cars.empty:
            st.success("Авторизация успешна!")
            st.markdown("---")
            
            # Если у клиента несколько машин по этому договору, даем выбрать нужную
            if len(user_cars) > 1:
                car_options = user_cars['Марка и Модель'].tolist()
                selected_car_name = st.selectbox("У вас несколько автомобилей. Выберите нужный для просмотра:", car_options)
            ...
            # --- ИНТЕРФЕЙС КАБИНЕТА (Отображается только после успешного входа) ---
            ...
            st.error("Договор с таким номером не найден. Пожалуйста, проверьте правильность ввода или обратитесь к вашему менеджеру.")
    else:
        st.info("💡 Пожалуйста, введите номер вашего договора комиссии выше, чтобы получить доступ к аналитике вашего автомобиля.")
